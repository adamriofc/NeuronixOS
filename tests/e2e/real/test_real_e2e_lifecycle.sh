...
#
# Label: [REAL-E2E-GATE] (Class: L4_HYBRID_ENGINE / L5_STORAGE_LIFECYCLE)
...
echo -e "${BOLD}${CYAN}║     Class: L4_HYBRID_ENGINE / L5_STORAGE | Zero-Simulation         ║${RESET}"
...
step_check "TOOLCHAIN_QEMU" "QEMU virtualization binary available ($QEMU_BIN)" \
    bash -c "test -x '$QEMU_BIN' || command -v '$QEMU_BIN' >/dev/null 2>&1"
...
step_check "QCOW2_METADATA" "Verified QCOW2 virtual size equals 10 GiB (10737418240 bytes)" \
    bash -c "\"$QEMU_IMG\" info '${TARGET_QCOW2}' | grep -q '10 GiB'"
...
if TARGET_ROOT="${TARGET_INSTALL_ROOT}" DRY_RUN=0 NEURONIX_IGNORE_PREFLIGHT=1 NEURONIX_SKIP_NIXOS_INSTALL=1 bash "${INSTALLER_BIN}" >"${SCRATCH_DIR}/install.log" 2>&1; then
    step_check "INSTALL_EXEC" "Installer completed with exit code 0 and generated core files" \
        bash -c "test -f '${TARGET_INSTALL_ROOT}/etc/nixos/flake.nix' && test -f '${TARGET_INSTALL_ROOT}/etc/nixos/configuration.nix' && test -f '${TARGET_INSTALL_ROOT}/etc/neuronix/release.json'"
else
...
step_check "TARGET_AST" "Target configuration.nix and flake.nix pass strict Nix AST parsing" \
    bash -c "nix-instantiate --parse '${TARGET_INSTALL_ROOT}/etc/nixos/configuration.nix' && nix-instantiate --parse '${TARGET_INSTALL_ROOT}/etc/nixos/flake.nix'"
...
step_check "GEN1_ACTIVE" "Generation 1 established with physical marker" \
    bash -c "test -f '${TARGET_INSTALL_ROOT}/etc/neuronix-marker-gen1' && readlink '${TEST_PROFILE}' | grep -q 'system-1-link'"
...
step_check "GEN2_ACTIVE" "Generation 2 mutation active with physical marker" \
    bash -c "test -f '${TARGET_INSTALL_ROOT}/etc/neuronix-marker-gen2' && readlink '${TEST_PROFILE}' | grep -q 'system-2-link'"
...
step_check "GEN1_RESTORED" "Active profile link verified pointing to Generation 1" \
    bash -c "readlink '${TEST_PROFILE}' | grep -q 'system-1-link'"
...
step_check "MARKER2_EVICTED" "Generation 2 physical marker strictly evicted post-rollback" \
    bash -c "! test -f '${TARGET_INSTALL_ROOT}/etc/neuronix-marker-gen2'"
...
echo -e "  Proof Class                   : L4_HYBRID_ENGINE / L5_STORAGE"
echo -e "  Evidence File                 : ${EVIDENCE_FILE}"
echo -e "${BOLD}═══════════════════════════════════════════════════════════════════${RESET}\n"

if [[ $FAILED -eq 0 ]]; then
    echo -e "${BOLD}${GREEN}✔ REAL E2E LIFECYCLE GATE VERIFIED (CLASS L4/L5 GREEN)${RESET}\n"
    exit 0
else
