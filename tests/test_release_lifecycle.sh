...
assert_check "Release proof offline validity" "python3 '${PROJECT_ROOT}/tools/verify_passport.py' '${PROJECT_ROOT}/dist/neuronix-os-v${CANONICAL_VERSION}.proof.json'"
...
assert_check "Release proof binds verification passport file hash" "python3 -c \"import json, hashlib; proof=json.load(open('${PROJECT_ROOT}/dist/neuronix-os-v${CANONICAL_VERSION}.proof.json')); h=hashlib.sha256(open('${PROJECT_ROOT}/dist/verification-passport.json', 'rb').read()).hexdigest(); assert proof['cryptographic_commitments']['verification_passport_sha256'] == h\""
assert_check "Release proof binds evidence graph file hash" "python3 -c \"import json, hashlib; proof=json.load(open('${PROJECT_ROOT}/dist/neuronix-os-v${CANONICAL_VERSION}.proof.json')); h=hashlib.sha256(open('${PROJECT_ROOT}/dist/evidence-graph.json', 'rb').read()).hexdigest(); assert proof['cryptographic_commitments']['evidence_graph_sha256'] == h\""
