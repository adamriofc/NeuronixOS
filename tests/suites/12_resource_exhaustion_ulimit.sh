...
assert_output_contains "(ulimit -n 256 && $TARGET_BIN version)" "$CANONICAL_VERSION" "Low fd limit preserves output integrity"
...
assert_output_contains "$TARGET_BIN version | head -n 1" "_____" "Piping to 'head -n 1' exits with clean pipe closing"
assert_output_contains "$TARGET_BIN help | head -n 5" "_____" "Piping to 'head -n 5' handles early SIGPIPE cleanly"
