...
assert_exit_code "$TARGET_BIN sandbox --help" 0 "neuronix sandbox --help exits 0"
assert_output_contains "$TARGET_BIN sandbox --help" "Shadow Micro-VM Sandbox" "Help manual displays Shadow Micro-VM title"
...
assert_exit_code "$TARGET_BIN sandbox --smoke-test" 0 "neuronix sandbox --smoke-test exits 0"
SMOKE_OUT=$($TARGET_BIN sandbox --smoke-test)
...
assert_exit_code "$TARGET_BIN sandbox --headless --smoke-test" 0 "Explicit headless smoke-test exits 0"
assert_output_contains "$TARGET_BIN sandbox --gui --dry-run" "GUI Display" "Option --gui sets GUI display mode"
assert_output_contains "$TARGET_BIN sandbox --headless --dry-run" "Headless (Console)" "Option --headless sets console mode"
assert_output_contains "$TARGET_BIN sandbox --timeout 30 --dry-run" "30 seconds" "Option --timeout parses valid positive integer"

# 20-26. Negative Bounds & Error Injection Handling
assert_exit_code "$TARGET_BIN sandbox --timeout 0" 1 "Option --timeout 0 is rejected with exit code 1"
assert_exit_code "$TARGET_BIN sandbox --mode synthetic --promote --smoke-test" 1 "Option --promote rejected under synthetic mode"
assert_exit_code "$TARGET_BIN sandbox --timeout invalid_abc" 1 "Non-numeric timeout is rejected with exit code 1"
assert_exit_code "$TARGET_BIN sandbox --unknown-vm-flag" 1 "Unknown flag for sandbox command is rejected with exit 1"
assert_stderr_contains "$TARGET_BIN sandbox --unknown-vm-flag" "tidak dikenali" "Error message emitted to stderr on invalid flag"
assert_exit_code "$TARGET_BIN sandbox --promote --dry-run" 0 "Option --promote combined with dry-run exits 0"
assert_exit_code "$TARGET_BIN sandbox /etc/nixos/configuration.nix extra_unexpected_arg" 1 "Multiple positional targets rejected"
