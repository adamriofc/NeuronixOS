...
# 18-22. Formal Proof Gatekeeper via MCP
...
assert_output_contains "echo '$VERIFY_OK_RES'" 'Formal Proof PASSED' "Valid package passes formal proof"
...
assert_output_contains "echo '$VERIFY_BAD_RES'" 'Formal Proof FAILED' "Invalid package fails formal proof"
...
# 26-30. Direct CLI Formal Verification Ergonomics
assert_exit_code "$TARGET_BIN verify hello" 0 "CLI verify hello returns exit 0"
assert_output_contains "$TARGET_BIN verify hello" "Formal Proof PASSED" "CLI verify displays success banner"
assert_exit_code "$TARGET_BIN verify nonexistent_fake_pkg_xyz" 1 "CLI verify nonexistent package exits 1"
assert_stderr_contains "$TARGET_BIN verify nonexistent_fake_pkg_xyz" "Formal Proof FAILED" "CLI verify displays rejection in stderr"
