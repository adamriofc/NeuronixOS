...
if [ "$DRY_RUN" -eq 0 ] && command -v nixos-generate-config >/dev/null 2>&1; then
  ...
  # Fallback hardware config for dry-run simulation or non-NixOS environments
...
elif [ -f "/etc/neuronix/version.nix" ]; then
  cp "/etc/neuronix/version.nix" "$CONFIG_DIR/version.nix"
fi
